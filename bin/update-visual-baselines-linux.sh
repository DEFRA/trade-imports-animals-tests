#!/bin/sh
# Regenerate the frozen main-suite visual baseline in a Linux container.
...
#
# Override PLAYWRIGHT_IMAGE, NODE_MODULES_VOLUME, MONGODB_URI, TRADE_IMPORTS_ANIMALS_BACKEND_URL, MAIN_FRONTEND_URL, or CONTAINER_USER to customise the container run.
set -eu
...
docker run --rm \
  ...
  -e TRADE_IMPORTS_ANIMALS_FRONTEND_BASE_URL="$MAIN_FRONTEND_URL" \
  --entrypoint npm \
  "$PLAYWRIGHT_IMAGE" \
  run _test_parity_main -- --grep @visual --update-snapshots "$@"
