...
test.describe('Accompanying document upload', () => {
  test('uploads an accompanying document and lands on the addresses page', async ({ journey, journeyContext, pages }) => {
    const options = {
    ...
    await test.step('upload the accompanying document', async () => {
      // Addresses is the page directly after accompanying documents — saves the upload without submitting.
      // saveAccompanyingDocuments()'s Save and continue click waits on virusScanComplete, so reaching
      // the addresses page already proves the upload + virus scan succeeded.
      await journey.toAddresses(options);
    });

    await test.step('assert the UI reflects a successful upload', async () => {
