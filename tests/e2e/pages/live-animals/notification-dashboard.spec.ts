...
test.describe('Import notification service dashboard', { tag: '@integration' }, () => {
  test('starts a journey at the promoted import-type filter and lists the draft', async ({
  ...
  test.describe('dashboard basics', () => {
    test('lands on the notification dashboard', { tag: '@smoke' }, async ({ liveAnimalsJourney: journey, liveAnimalsPages: pages }) => {
    ...
    test('allows creating a new notification', async ({ liveAnimalsJourney: journey, liveAnimalsPages: pages }) => {
      await journey.toNotificationDashboard();
      await pages.notificationDashboard.btnCreateNewNotification.click();
      await expect(pages.importType.heading).toBeVisible();
    });

    test('displays the notification list and result count', async ({ liveAnimalsApiJourney: apiJourney, liveAnimalsPages: pages }) => {
    ...
    test('displays details on a notification card', async ({ liveAnimalsJourney: journey, journeyContext, liveAnimalsPages: pages }) => {
      test.slow();
  ...
  test.describe('notification card actions by status', () => {
    test('shows resume, copy and delete actions for a draft notification', async ({
      liveAnimalsPages: pages,
    ...
    test('shows view, copy and amend actions for a submitted notification', async ({
      liveAnimalsPages: pages,
      liveAnimalsApiJourney: apiJourney,
    }) => {
    ...
    test(
      ...
      { tag: '@smoke' },
      async ({ liveAnimalsPages: pages, liveAnimalsJourney: journey, journeyContext }) => {
        test.slow();
        await journey.submitNotification();
