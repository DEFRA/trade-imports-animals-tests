...
test.describe('Origin of the import page', { tag: ['@integration', '@duplicated-in-frontend'] }, () => {
  ...
  test('shows the country selector on load', async ({ liveAnimalsPages: pages }) => {
    await expect(pages.originOfImport.countryOfOrigin).toBeVisible();
  });

  test('accepts valid origin details', async ({ liveAnimalsPages: pages }) => {
  ...
  test('shows an error summary when submitted without a country', async ({ liveAnimalsPages: pages }) => {
    await pages.originOfImport.saveAndContinue.click();
