...
test.describe('Notification persistence', { tag: ['@compose', '@integration', '@mongodb'] }, () => {
  ...
  test('persists notification with defaults (after full journey completion*)', async ({ journey, journeyContext }) => {
    ...
    try {
      ...
      expect(doc.placeOfOrigin.address.addressLine1).toBe("7 Drover's Way");
      expect(doc.placeOfOrigin.address.country).toBe('Germany');
      expect(doc.consignor?.name).toBe(CONSIGNOR_NAME);
      expect(doc.consignor?.address.addressLine1).toBe("1 Drover's Way");
      expect(doc.consignor?.address.addressLine2).toBe('Unit 1');
      expect(doc.consignor?.address.addressLine3).toBeUndefined();
      expect(doc.consignor?.address.country).toBe('Ireland');
      expect(doc.consignee.name).toBe(CONSIGNEE_NAME);
      expect(doc.consignee.address.addressLine1).toBe("2 Drover's Way");
      expect(doc.consignee.address.country).toBe('France');
      expect(doc.importer.name).toBe(IMPORTER_NAME);
      expect(doc.importer.address.addressLine1).toBe("3 Drover's Way");
      expect(doc.importer.address.country).toBe('Germany');
      expect(doc.destination?.name).toBe(DESTINATION_NAME);
      expect(doc.destination?.address.addressLine1).toBe("4 Drover's Way");
      expect(doc.destination?.address.addressLine2).toBe('Unit 4');
      ...
      expect(doc.transport.transporter?.address.addressLine1).toBe("5 Drover's Way");
      expect(doc.transport.transporter?.address.addressLine2).toBe('Unit 5');
      expect(doc.transport.transporter?.address.addressLine3).toBeUndefined();
      expect(doc.transport.transporter?.address.country).toBe('Ireland');
      expect(doc.transport.transporter?.approvalNumber).toBe('APR-0005');
      expect(doc.transport.transporter?.type).toBe('PRIVATE');
      expect(doc.consignment?.name).toBe(CONTACT_ADDRESS_NAME);
      expect(doc.consignment?.address.addressLine1).toBe("6 Drover's Way");
      expect(doc.consignment?.address.addressLine2).toBe('Unit 6');
      expect(doc.consignment?.address.addressLine3).toBeUndefined();
      expect(doc.consignment?.address.country).toBe('France');
