...
test.describe('Notification dashboard sort', () => {
  ...
  test('default sort option is "Arrival (newest to oldest)"', async ({ liveAnimalsPages: pages }) => {
  ...
  test('sort dropdown contains all four expected options', async ({ liveAnimalsPages: pages }) => {
  ...
  for (const value of Object.values(sortByValues)) {
    test(`selecting "${value}" submits and reloads without error`, async ({ liveAnimalsPages: pages }) => {
