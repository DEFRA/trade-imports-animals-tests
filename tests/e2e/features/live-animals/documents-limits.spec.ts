...
test.describe('Documents limits', { tag: ['@integration', '@duplicated-in-frontend'] }, () => {
  test('accepts a tenth document and rejects an eleventh with the maximum-documents error', async ({
  ...
  test('accepts a 10 MB PDF and rejects the same real file at one byte over', async ({
    liveAnimalsJourney: journey,
    liveAnimalsPages: pages,
  }, testInfo) => {
    test.slow();
