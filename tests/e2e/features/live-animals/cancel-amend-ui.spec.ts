...
test.describe('Cancel amendment through the UI', { tag: ['@integration'] }, () => {
  test.describe('from an amending notification', () => {
    test.beforeEach(async ({ liveAnimalsApiJourney: apiJourney, notificationActions }) => {
      const created = await apiJourney.createAmendNotification();
      await notificationActions.toNotificationView(created.id);
    });

    test('shows the Cancel amendment link while the notification is amending', async ({ liveAnimalsPages: pages }) => {
    ...
    test('opens the confirmation page when Cancel amendment is selected', async ({ liveAnimalsPages: pages, journeyContext }) => {
    ...
    test('No returns to the notification view with the amendment still in progress', async ({
  ...
  test(
    ...
    { tag: '@smoke' },
    async ({ liveAnimalsPages: pages, liveAnimalsApiJourney: apiJourney, notificationActions }) => {
      const created = await apiJourney.createAmendNotification();
      await notificationActions.toNotificationView(created.id);
