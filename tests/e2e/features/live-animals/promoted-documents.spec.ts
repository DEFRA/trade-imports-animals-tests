...
test.describe('Promoted accompanying documents integration', { tag: ['@compose', '@integration'] }, () => {
  test('uploads, scans, downloads and removes a document through the real uploader', async ({
  ...
  test('rejects an unsupported file type without adding a document', async ({ liveAnimalsJourney: journey, liveAnimalsPages: pages }) => {
    await journey.toAccompanyingDocuments();
