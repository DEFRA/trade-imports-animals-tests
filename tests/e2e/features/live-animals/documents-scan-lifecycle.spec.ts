...
test.describe('Documents scan lifecycle', { tag: ['@integration', '@duplicated-in-frontend'] }, () => {
  ...
  test('virus-named upload: accepted with Checking, then Virus found with error summary and no view link', async ({
  ...
  test('clean upload: shows Checking with no view link, then Safe with a view link', async ({ liveAnimalsPages: pages }) => {
    test.slow();
