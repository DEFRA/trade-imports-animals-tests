...
import { commodityTypes } from '@main-domain/constants/commodity-types';
...
test.describe('Commodity details', () => {
  test.beforeEach(async ({ apiJourney, pages }) => {
    const created = await apiJourney.createUpToPage('commodityDetails');
    await apiJourney.resumeInUi(created.referenceNumber, pages.commodityDetails);
  });

  test('shows system-generated reference number', async ({ journeyContext, pages }) => {
    const referenceNumber = await pages.commodityDetails.referenceNumber.textContent();
