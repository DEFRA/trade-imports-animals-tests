...
import { TEN_MB_BYTES, ABOVE_PAYLOAD_CAP_BYTES, OVERSIZE_FILE_MESSAGE } from '@main-resources/file-upload/constants';
...
test.describe('Accompanying documents - without JavaScript', { tag: ['@integration', '@no-js'] }, () => {
  ...
  test('shows manual refresh link instead of polling when virus scan is pending', async ({ pages }) => {
  ...
  test('rejects a file one byte over the 10 MB cap with a server-rendered inline error', async ({ pages }, testInfo) => {
  ...
  test('rejects a file above the route payload cap by re-rendering the page with an inline error', async ({ pages }, testInfo) => {
